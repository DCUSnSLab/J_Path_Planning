...
class Map(object):
	...
	def add_circles_to_exploration_buffer(self, poses, radii, exp_coeff=None):
		...
		print("radii", radii)
		print("radii type", type(radii))
		print("self map info", self.map_info)
		print("self map info type", type(self.map_info))
...
def world_to_map(poses, map_info):
	...
	print("pose len", poses.size)
	if poses.size == 1:
		pass
	else:
		print("pose type", type(poses))
		print("x", map_info.origin.position.x)
		print("x type", type(map_info.origin.position.x))
