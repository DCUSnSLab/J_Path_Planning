...
class PurePursuit(object):
	...
	"""
	def __init__(self):
		self.trajectory_topic = rospy.get_param("~trajectory_topic", "/trajectory/current")
		self.odom_topic       = rospy.get_param("~odom_topic", "/pf/pose/odom")
		self.lookahead        = rospy.get_param("~lookahead", 1.5)
		self.max_reacquire    = rospy.get_param("~max_reacquire", 3.0)
		self.speed            = float(rospy.get_param("~speed", 1.5))
		self.wrap             = bool(rospy.get_param("~wrap", 0))
		wheelbase_length      = float(rospy.get_param("~wheelbase", 0.25))
